/**
 * Applies migrations/add_ai_chat_tables.sql only (no full reset).
 * Use when you need conversations/messages tables without re-running init-db.sql.
 *
 *   npm run db:ai-chat
 */
...
async function main() {
  const filePath = path.join(__dirname, "..", "migrations", "add_ai_chat_tables.sql");
  if (!fs.existsSync(filePath)) {
    console.error("Missing:", filePath);
    process.exit(1);
  }
  await runSqlFile("AI chat tables", filePath);
  await pool.end();
}
