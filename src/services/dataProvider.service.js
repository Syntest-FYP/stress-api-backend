...
const resolveExecutionStep = async (step, environmentId, executionContext = {}) => {
  ...
  if (resolvedStep.body && typeof resolvedStep.body === 'string') {
    ...
    resolvedStep.body = JSON.parse(await resolvePlaceholders(JSON.stringify(resolvedStep.body), environmentId, executionContext));
  }
  
  if (resolvedStep.headers) {
    resolvedStep.headers = JSON.parse(await resolvePlaceholders(JSON.stringify(resolvedStep.headers), environmentId, executionContext));
  }
